...
MODEL_FILENAME = 'v33_model.txt'


class ModelV33(BaseModel):
	...
	def load_data(self, filenames):
		...
		df       = super(ModelV33, self).reduce_mem_usage(df)
	...
	def create_folds(self, fold_name, seed):
		...
		dtr, dte = super(ModelV33, self).create_fold(data, seed)
	...
	def train(self, train, test, feature_list, is_eval, **params):
		...
		return super(ModelV33, self).train_lgb(X, y, Xte, yte, **params)
	...
	def evaluate(self, test, feature_list, is_eval, model):
		...
		return super(ModelV33, self).evaluate_lgb(Xte, yte, model)

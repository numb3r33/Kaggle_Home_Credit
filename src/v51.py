...
class Modelv51(BaseModel):
    ...
    def load_data(self, filenames):
        ...
        df       = super(Modelv51, self).reduce_mem_usage(df)
    ...
    def create_folds(self, fold_name, seed):
        ...
        dtr, dte = super(Modelv51, self).create_fold(data, seed)
    ...
    def prepare_features(self, fold_name):
        ...
        if not os.path.exists(os.path.join(basepath, self.params['output_path'] + self.params['run_name'] + f'bureau_{fold_name}train.pkl')):
            ...
            data = super(Modelv51, self).fill_infrequent_values(data)
    ...
    def train(self, train, test, feature_list, is_eval, TARGET_NAME='TARGET', **params):
        ...
        return super(Modelv51, self).train_lgb(X, y, Xte, yte, **params) if self.params['model_type'] == 'LGB'\
               else super(Modelv51, self).train_xgb(X, y, Xte, yte, **params)
    ...
    def evaluate(self, test, feature_list, is_eval, model, TARGET_NAME='TARGET'):
        ...
        return super(Modelv51, self).evaluate_lgb(Xte, yte, model) if self.params['model_type'] == 'LGB'\
               else super(Modelv51, self).evaluate_xgb(Xte, yte, model)
    ...
    def get_sample(self, train, sample_size=.5, seed=SEED):
        ...
        """
        return super(Modelv51, self).get_sample(train, sample_size, seed=SEED)
    ...
    def oof_preds(self, train, feature_list, model):
        ...
        return super(Modelv51, self).oof_preds(X, y, model)
...
if __name__ == '__main__':
    ...
    if args.p:
        ...
        if not is_eval:
            ...
            HOLDOUT_SCORE = (0.7932 + 0.7934 + 0.7895) / 3

...
class Modelv103(BaseModel):
    ...
    def load_data(self, filenames):
        ...
        df       = super(Modelv103, self).reduce_mem_usage(df)

        return df
    
    def reduce_mem_usage(self, df):
        return super(Modelv103, self).reduce_mem_usage(df)
    ...
    def preprocess(self):
        ...
        if not os.path.exists(os.path.join(basepath, self.params['output_path'] + 'feature_groups/' + f'current_application_train.pkl')):
            ...
            data = super(Modelv103, self).fill_infrequent_values(data)
    ...
    def get_features(self, train, test, compute_ohe):
        ...
        if os.path.exists(os.path.join(basepath, self.params['output_path'] + f'{self.params["data_folder"]}pca.pkl')):
            ...
            pca_components = super(Modelv103, self).add_pca_components(data.copy(), PCA_PARAMS)
        ...
        if compute_ohe:
            data = super(Modelv103, self).prepare_ohe(data, OHE_COLS, drop_col=True)
        else:
    ...
    def train(self, train, test, feature_list, is_eval, model, TARGET_NAME='TARGET'):
        ...
        return super(Modelv103, self).train_sklearn(X, y, Xte, yte, model)
    ...
    def evaluate(self, test, feature_list, is_eval, model, TARGET_NAME='TARGET'):
        ...
        return super(Modelv103, self).evaluate_sklearn(Xte, yte, model)
    ...
    def cross_validate(self, train, feature_list, model, seed, TARGET_NAME='TARGET'):
        ...
        return super(Modelv103, self).cross_validate_sklearn(Xtr, 
                                                    ytr,
                                                    model,
                                                    seed
                                                    )
    ...
    def scale_dataset(self, data, feature_list):
        return (data.loc[:, feature_list] - data.loc[:, feature_list].mean()) / (data.loc[:, feature_list].std())
    ...
    def oof_preds(self, train, test, feature_list, model, TARGET_NAME='TARGET'):
        ...
        return super(Modelv103, self).oof_preds(X, y, Xte, model)
...
if __name__ == '__main__':
    ...
    if args.p:
        ...
        if os.path.exists(os.path.join(basepath, output_path + f'{model_identifier}_model.pkl')):
            print('Loading model from disk ...')
            model = joblib.load(os.path.join(basepath, output_path + f'{model_identifier}_model.pkl'))
            yhold = test.TARGET
            ...
            model, hold_preds = m.train(train, test, feature_list, is_eval=True, model=model)
            
            if not is_sample:
                joblib.dump(model, os.path.join(basepath, output_path + f'{model_identifier}_model.pkl'))

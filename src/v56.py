...
class Modelv56(BaseModel):
    ...
    def load_data(self, filenames):
        ...
        df       = super(Modelv56, self).reduce_mem_usage(df)
    ...
    def create_folds(self, fold_name, seed):
        ...
        dtr, dte = super(Modelv56, self).create_fold(data, seed)
    ...
    def prepare_features(self, fold_name):
        ...
        if not os.path.exists(os.path.join(basepath, self.params['output_path'] + self.params['run_name'] + f'current_application_{fold_name}train.pkl')):
            ...
            data = super(Modelv56, self).fill_infrequent_values(data)
    ...
    def train(self, train, test, feature_list, is_eval, TARGET_NAME='TARGET', **params):
        ...
        return super(Modelv56, self).train_lgb(X, y, Xte, yte, **params) if self.params['model_type'] == 'LGB'\
               else super(Modelv56, self).train_xgb(X, y, Xte, yte, **params)
    ...
    def evaluate(self, test, feature_list, is_eval, model, TARGET_NAME='TARGET'):
        ...
        return super(Modelv56, self).evaluate_lgb(Xte, yte, model) if self.params['model_type'] == 'LGB'\
               else super(Modelv56, self).evaluate_xgb(Xte, yte, model)
    ...
    def get_sample(self, train, sample_size=.5, seed=SEED):
        ...
        """
        return super(Modelv56, self).get_sample(train, sample_size, seed=SEED)
    ...
    def get_features(self, train, test, compute_ohe):
        ...
        if compute_ohe:
            data = super(Modelv56, self).prepare_ohe(data, OHE_COLS, drop_col=True)
        else:
    ...
    def oof_preds(self, train, feature_list, model):
        ...
        return super(Modelv56, self).oof_preds(X, y, model)
...
if __name__ == '__main__':
    ...
    if args.p:
        ...
        oof_preds_filenames = [os.path.join(basepath, output_path + run_name + f'test_preds_{MODEL_FILENAME}_{model_type}_5432_F0_0.7930083374342244.pkl'),
                               os.path.join(basepath, output_path + run_name + f'test_preds_{MODEL_FILENAME}_{model_type}_6280_F1_0.7938754393581124.pkl'),
                               os.path.join(basepath, output_path + run_name + f'test_preds_{MODEL_FILENAME}_{model_type}_5158_F2_0.7890351271935672.pkl')
                              ]
        ...
        rank_average = (fold0_preds + fold1_preds + fold2_preds) / 3

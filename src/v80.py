...
class Modelv80(BaseModel):
    ...
    def load_data(self, filenames):
        ...
        df       = super(Modelv80, self).reduce_mem_usage(df)
    ...
    def preprocess(self):
        ...
        if not os.path.exists(os.path.join(basepath, self.params['output_path'] + 'feature_groups/' + f'current_application_train.pkl')):
            ...
            data = super(Modelv80, self).fill_infrequent_values(data)
    ...
    def get_features(self, train, test, compute_ohe):
        ...
        # add pca components
        if os.path.exists(os.path.join(basepath, self.params['output_path'] + 'feature_groups/' + f'pca.pkl')):
            pca_components = pd.read_pickle(os.path.join(basepath, self.params['output_path'] + 'feature_groups/' + f'pca.pkl'))
        else:
            pca_components = super(Modelv80, self).add_pca_components(data, PCA_PARAMS)
            pca_components.to_pickle(os.path.join(basepath, self.params['output_path'] + 'feature_groups/' + f'pca.pkl'))
        ...
        if compute_ohe:
            data = super(Modelv80, self).prepare_ohe(data, OHE_COLS, drop_col=True)
        else:
    ...
    def train(self, train, test, feature_list, is_eval, TARGET_NAME='TARGET', **params):
        ...
        return super(Modelv80, self).train_lgb(X, y, Xte, yte, **params)
    ...
    def evaluate(self, test, feature_list, is_eval, model, TARGET_NAME='TARGET'):
        ...
        return super(Modelv80, self).evaluate_lgb(Xte, yte, model)
    ...
    def cross_validate(self, train, feature_list, params, cv_adversarial_filepath=None, TARGET_NAME='TARGET'):
        ...
        return super(Modelv80, self).cross_validate(Xtr, ytr, params, cv_adversarial_filepath=cv_adversarial_filepath)
...
if __name__ == '__main__':
    ...
    if args.p:
        ...
        itr    = np.array(list(set(np.arange(m.n_train)) - set(ite)))
